...
class AnswerStopper(StoppingCriteria):
    # Combines both stopping patterns into one regex using '|' (OR)
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor, **kwargs) -> bool:  # type: ignore[override]
        # Get all generated token IDs, not just the tail
        generated_ids = input_ids[0][self.start_idx:]
        if len(generated_ids) == 0:
            return False

        # Decode the entire generated text
        generated_text = self.tok.decode(generated_ids, skip_special_tokens=True)
        
        # Search for the pattern anywhere in the generated text
        return bool(self.pattern.search(generated_text))
...
def func_infer_one(
    ...
    func_map = ["<add>", "<subtract>", "<multiply>", "<divide>", "<power>", "<sqrt>", "<log>", "<ln>", "<lcm>", "<gcd>", "<remainder>", "<choose>", "<permutate>"]
    cur_gen = ""
    cur_gen_with_func = ""
    func_calls: List[str] = []
    start_len: List[int] = []
    ...
    for _ in range(max_gen_len):
        ...
        # Stop after the full answer, handling optional currency symbols ($) and other text.
        if re.search(r"(?:####|answer is)\s*\$?[-+]?[\d,]+(?:\.\d+)?.*?\.\n\n", cur_gen, re.IGNORECASE):
            break       
